...
- Max trial time(Should be same for both no go and go trials)
...
class StopAndGo(object):
    ...
    def __init__(self, screen=None, monkey_name=None, g_params=None, m_params=None, arm_used=None, clipart=None):
        ...
        #params
        self.sidesNum = 0
        self.ratio = 0 #ratio of go trials to no go trials (ex. 50 would be 50:50, 60 would be 60:40, etc)(go:nogo)
        self.timeBetweenTrials = 0#seconds
        self.noGoDelayPeriod = 0
        self.abortTrialTimeForGo = 0
        self.rewardDelay = 0
        self.relayPumpTime = 0
    ...
    def on_loop(self):
        if self.g_params['STIMULI_COLORS'] != 'normal':
            ...
            color = GREEN
        self.stim_x += (self.get_x_velocity()) * (1/FPS)
