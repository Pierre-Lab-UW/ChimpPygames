...
==
3 MTS trials, then one DMTS trial with 3 second delay. Progress to criterion based on success of DMTS trial
"""
...
class MTSseq3(object):
    ...
    def __init__(self, screen=None, monkey_name=None, g_params=None, m_params=None, arm_used=None, clipart=None):
        ...
        self.sample_x = 0
        self.sample_y = 0
        self.pos = {'left': (275, 450), 'right': (525, 450)}
        self.sample_i = None
        ...
        # #
            ...
            if self.match.rect.contains((touch_x, touch_y, 1, 1)):
                write_ln(self.filepath_to_data,
                         [self.monkey_name, time.strftime('%Y-%m-%d'), time.strftime('%H:%M'), self.arm_used,
                          ...
                          self.clipart_this_trial, self.sample_pos, self.num_matches, touch_x, touch_y, 1])
                if self.trial_style[self.trial_style_i] == 'delay':
            ...
            # #
                ...
                write_ln(self.filepath_to_data,
                         [self.monkey_name, time.strftime('%Y-%m-%d'), time.strftime('%H:%M'), self.arm_used,
                          self.task_name, self.trial, self.trial_style[self.trial_style_i],
                          self.clipart_this_trial, self.sample_pos, self.num_matches, touch_x, touch_y, 0])
                if self.trial_style[self.trial_style_i] == 'delay':
                    with open(self.filepath_to_progress, 'a') as f:
                        f.writelines(str(0) + '\n')
                self.check_for_progression()
