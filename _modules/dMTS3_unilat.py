...
class dMTS3_unilat(object):
    ...
    def new_trial(self):
        ...
        (self.sample_x, self.sample_y) = (200,330)
    ...
    def on_touch(self, touch_x=None, touch_y=None):
        ...
        if self.on_start_screen:
            if self.sample.rect.contains((touch_x, touch_y, 1, 1)):
                ...
                with open (self.filepath_to_side) as s:
                    last_3 = s.readlines()[-3:]
                if last_3 == ['top\n', 'top\n', 'top\n']:
                    self.sample_pos = 'bottom'
                    self.nonmatch_pos = 'top'
                    log('sample {} next trial!'.format(self.sample_pos))
                elif last_3 == ['bottom\n', 'bottom\n', 'bottom\n']:
                    self.sample_pos = 'top'
                    self.nonmatch_pos = 'bottom'
                    log('sample {} next trial!'.format(self.sample_pos))
                else:
        ...
        # #
            ...
            if self.match.rect.contains((touch_x, touch_y, 1, 1)):
                ...
                write_ln(self.filepath_to_side, [self.sample_pos])
                if self.trial_style[self.trial_style_i] == 'delay':
