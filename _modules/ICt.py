"""
Inhibitory control - testing version
==
...
==
New in 2023
+  one trial length per session (parameter-determined)
+  variable delay
==
As part of monkeys' weekly testing battery Summer2023
"""
...
class ICt(object):
    ...
    def __init__(self, screen=None, monkey_name=None, g_params=None, m_params=None, arm_used=None, clipart=None):
        ...
        self.filepath_to_movement_delays = os.path.join('_progress', self.monkey_name, 'ICt-times-remaining.txt'.format(time.strftime('%Y'), time.strftime('%W')))
        ...
        self.movement_delays_ix = 0
        self.movement_delay_this_problem = 0

        self.presentation_start = 0
        self.time_on_presentation = 0

        self.learning_trials_this_problem = int(self.m_params[self.monkey_name]['ICttrials'])
        self.choice_start = 0
    ...
    def movement_delayio(self):

        # create movement_delays file if it doesn't already exist
        if not os.path.isfile(self.filepath_to_movement_delays):
            possible_delays = self.m_params[self.monkey_name]['ICttimes'].split('-')
            ic_weekly_delays = []
            for delay_value in possible_delays:
                ic_weekly_delays += [delay_value]*(int(self.m_params[self.monkey_name]['ICtproblems'])//len(possible_delays))
            with open(self.filepath_to_movement_delays, 'w+') as f:
                for delay in ic_weekly_delays:
                    f.write(delay + '\n')

        # read in list of possible numbers of movement delays, remove empty lines
        # #
        with open(self.filepath_to_movement_delays, 'r') as f:
            self.movement_delays = f.readlines()
        without_blank_lines = []
        for i in self.movement_delays:
            if i != ' \n':
                without_blank_lines.append(i)
        self.movement_delays = without_blank_lines[:]

        self.movement_delays_ix = random.randint(0, len(self.movement_delays) - 1)
        self.movement_delay_this_problem = int(self.movement_delays[self.movement_delays_ix].replace('\n', ''))

    def prep_new_problem(self):
        self.trial = 0

        self.movement_delayio()
        ...
        top = int((self.g_params['SCREEN_H']) * .2) #.15 for SQM adjusted from .2
        bottom = int(self.g_params['SCREEN_H'] * .7) #.55 for SQM adjusted from .7
        ...
        if starting_pos_this_problem == 'topleft':
            self.problem = ['topleft']*self.learning_trials_this_problem + ['bottomleft']
        elif starting_pos_this_problem == 'bottomleft':
            self.problem = ['bottomleft']*self.learning_trials_this_problem + ['topleft']
        elif starting_pos_this_problem == 'topright':
            self.problem = ['topright']*self.learning_trials_this_problem + ['bottomright']
        elif starting_pos_this_problem == 'bottomright':
            self.problem = ['bottomright']*self.learning_trials_this_problem + ['topright']
    ...
    def new_trial(self):
        ...
        if not os.path.isfile(self.filepath_to_data):
            header = [
                'monkey_name', 'date', 'time', 'arm', 'task_name', 'trial',
                'sample_image', 'delay', 'correct_x', 'correct_y', 'correct_loc',
                'set_type', 'touch_x', 'touch_y', 'timestamp', 'choice_latency',
                'test_trial', 'correct'
            ]
    ...
    def on_loop(self):
        if self.on_start_screen:
            ...
            if self.trial < len(self.problem)-1 or (pg.time.get_ticks()-self.presentation_start) > self.movement_delay_this_problem:
    ...
    def on_touch(self, touch_x=None, touch_y=None):
        if self.on_start_screen:
            ...
            if math.sqrt((touch_x-flicker_center[0])**2 + (touch_y-flicker_center[1])**2) < self.stim_size//2:
                if pg.time.get_ticks() - self.presentation_start > 100 and math.sqrt((touch_x-flicker_center[0])**2 + (touch_y-flicker_center[1])**2) < self.stim_size//2:
                    self.time_on_choice_screen = pg.time.get_ticks() - self.presentation_start
                    data = [
                        self.monkey_name, time.strftime('%Y-%m-%d'), time.strftime('%H:%M'), self.arm_used,
                        self.task_name, self.trial, self.clipart_this_problem, self.movement_delay_this_problem,
                        flicker_center[0], flicker_center[1], self.problem[self.trial-1],
                        self.learning_trials_this_problem,
                        '-'.join(self.x_touches), '-'.join(self.y_touches), '-'.join(self.timestamps),
                        self.time_on_choice_screen, len(self.problem)==self.trial, 1
                    ]
    ...
    def check_for_progression(self):
        ...
        """
        with open(self.filepath_to_movement_delays, 'w') as f:
            for line_ix, line in enumerate(self.movement_delays):
                if line_ix != self.movement_delays_ix and line != '':
                    f.write(line)
        if not self.progressed:
            ...
            with open(self.filepath_to_progress, 'r') as f:
                ...
                trials_to_check_criterion = int(self.m_params[self.monkey_name]['ICtproblems'])
