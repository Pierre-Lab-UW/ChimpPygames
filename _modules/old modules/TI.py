...
class TI(object):
    ...
    def new_trial(self):
        ...
        # #
        filename = os.path.join('_progress', self.monkey_name, 'testing_progress', 'TI-set-ix-{}week{}.txt'.format(time.strftime('%Y'), time.strftime('%W')))
        if not os.path.isfile(filename):
            write_ln(filename, ['1'])
        with open(filename, 'r') as f:
            self.set = int(f.read())
        
        # if lots of time has elapsed since last TI trial, reset set. 
        #   create timestamp if one doesn't already exist
        # #
        ...
        with open(os.path.join('_progress', self.monkey_name, 'TI-set-timestamp.txt'), 'r') as f:
            try:
                ...
                timestamp = 0
        if (time.time() - timestamp) > (int(self.m_params[self.monkey_name]['TIreset'])):
            self.reset_set()
        ...
        # if animal has been on same learning phase for too long, reset set
        # #
        if self.phase == 'learning' and len(self.set_progress)>=60:
            self.reset_set()

        # if enough trials have been done to check for criterion, and animal has met criterion
        # #
        if self.phase == 'learning':
            if len(self.set_progress) == 11:
                trials_to_check_criterion = 11
                trials_to_achieve_criterion = 7
            if len(self.set_progress) == 12:
                trials_to_check_criterion = 12
                trials_to_achieve_criterion = 8
            if len(self.set_progress) == 14:
                trials_to_check_criterion = 14
                trials_to_achieve_criterion = 9
            if len(self.set_progress) == 16:
                trials_to_check_criterion = 16
                trials_to_achieve_criterion = 10
            if len(self.set_progress) == 19:
                trials_to_check_criterion = 19
                trials_to_achieve_criterion = 12
            if len(self.set_progress) >= 21:
                trials_to_check_criterion = 21
                trials_to_achieve_criterion = 14
            if len(self.set_progress) in [11, 12, 14, 16, 19] or len(self.set_progress)>=21:
                log(
                ...
                if sum(self.set_progress[-trials_to_check_criterion:]) >= trials_to_achieve_criterion+1:
                    with open(self.filepath_to_progress, 'w') as f:
                        f.truncate(0)
                    self.set_progress = []
                    self.reset_set()
                if sum(self.set_progress[-trials_to_check_criterion:]) >= trials_to_achieve_criterion:
                    with open(self.filepath_to_progress, 'w') as f:
                        f.truncate(0)
                    self.set_progress = []
                    self.phase = 'testing'
