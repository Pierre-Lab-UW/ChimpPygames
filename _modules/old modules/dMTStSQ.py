...
+  how to win :: one clipart is the sample, touching it reveals a sample and other clipart after one of four delays.
     correct response is to touch sample again
==
As part of monkeys' weekly test Fall2021
"""
...
class dMTStSQ(object):
    ...
    def __init__(self, screen=None, monkey_name=None, g_params=None, m_params=None, arm_used=None, clipart=None):
        ...
        self.filepath_to_delay = os.path.join('_progress', self.monkey_name, 'testing_progress', 'dMTStSQ-delays-remaining-{}week{}.txt'.format(time.strftime('%Y'), time.strftime('%W')))
        ...
        self.delay_lengths = None    # all possible delay lengths read from file
        self.delay_timer = None
        self.delay_ix = None     # line from list of possible delays that is being used this trial
        self.delay_this_trial = None
    ...
    def new_trial(self):
        ...
        if not os.path.isfile(self.filepath_to_data):
            write_ln(self.filepath_to_data, ['monkey_name', 'date', 'time', 'arm', 'task_name', 'trial',
                                             'sample_image', 'delay', 'sample_pos', 'touch_x', 'touch_y', 'correct'])
        ...
        if not os.path.isfile(self.filepath_to_delay):
            dMTStSQ_weekly_trials = ['0']*200 + ['1']*50 + ['3']*50 + ['5']*50 + ['8']*50
            with open(self.filepath_to_delay, 'w') as f:
                for trial in dMTStSQ_weekly_trials:
                    f.write(trial + '\n')
        ...
        self.stim_size = int(self.m_params[self.monkey_name]['dMTStsize'])      # get stim_size
        ...
        self.delay_this_trial = int(self.delay_lengths[self.delay_ix].replace('\n', ''))*1000
    ...
    def on_loop(self):
        ...
        if self.on_delay_screen:
            self.screen.bg.fill(BLACK)
            self.screen.fg.blit(self.screen.bg, (0, 0))
            pg.event.clear()
            if pg.time.get_ticks() - self.delay_timer > self.delay_this_trial:
                self.on_delay_screen = False
        if not self.on_start_screen and not self.on_delay_screen:
    ...
    def on_touch(self, touch_x=None, touch_y=None):
        ...
        # #
            ...
            if self.match.rect.contains((touch_x, touch_y, 1, 1)):
                write_ln(self.filepath_to_data,
                         [self.monkey_name, time.strftime('%Y-%m-%d'), time.strftime('%H:%M:%S'), self.arm_used,
                          self.task_name, self.trial, self.clipart_this_trial, self.delay_this_trial, self.sample_pos, touch_x, touch_y, 1])
            ...
            # #
                ...
                write_ln(self.filepath_to_data,
                         [self.monkey_name, time.strftime('%Y-%m-%d'), time.strftime('%H:%M:%S'), self.arm_used,
                          self.task_name, self.trial, self.clipart_this_trial, self.delay_this_trial, self.sample_pos, touch_x, touch_y, 0])
                with open(self.filepath_to_progress, 'a') as f:
                    f.writelines(str(0) + '\n')
                self.check_for_progression()
                return 'timeout'
    ...
    def check_for_progression(self):
        ...
        if not self.progressed:
            ...
            with open(self.filepath_to_progress, 'r') as f:
                ...
                trials_to_check_criterion = int(self.m_params[self.monkey_name]['dMTSttrials'])
                trials_to_achieve_criterion = int(self.m_params[self.monkey_name]['dMTStcriterion'])
                if ((len(progress) >= trials_to_check_criterion) and \
                        (sum(progress[-trials_to_check_criterion:]) >= trials_to_achieve_criterion)) \
                        or len(self.delay_lengths)==1:
                    ...
                    with open(self.filepath_to_progress, 'w') as f:
                        f.truncate(0)
                    # os.remove(self.filepath_to_delay)     # delete trials to do list
